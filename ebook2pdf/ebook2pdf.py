...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.label1_1 = QLabel('(0,0) '+str(self.posX1)+','+str(self.posY1), self)
        self.label2 = QLabel('이미지 우측하단 좌표 ==> ', self)
        self.label2_1 = QLabel('(0,0) '+str(self.posX2)+','+str(self.posY2), self)
    ...
    def 좌측상단_좌표_클릭(self):
        ...
        def on_click(x, y, button, pressed):
            ...
            print('xClick UpLeft: %s, Position: (%s, %s), Pressed: %s ' % (button, x, y, pressed))
            if not pressed:
                return False
    ...
    def 우측하단_좌표_클릭(self):
        ...
        def on_click(x, y, button, pressed):
            ...
            print('xClick DownRight: %s, Position: (%s, %s), Pressed: %s ' % (button, x, y, pressed))
            if not pressed:
                return False
    ...
    def 속도_변경(self):
        # slider는 정수 이므로, speed취급단위 0.1초로 정리
    ...
    def btn_click(self):
        def btn_msg01(self):
            localmsg01 = "xSTEP: Start btn msg01"
            # self.stat.setText('STEP:joe click')
            self.stat.setText( localmsg01 )
            # self.stat.setText('STEP:joe click')
            self.stat.show()
            print(localmsg01)

        def btn_msg02(self):
            localmsg02 = 'xSTEP: Start btn msg02'
            self.sign.setText(localmsg02)
            self.sign.repaint()
            self.button3.setText(localmsg02)
        ...
        print("xBTN:set pos Upleft: " + str(self.posX1) + ":" + str(self.posY1) )
        print("xBTN:set pos DownRight: " + str(self.posX2) + ":" + str(self.posY2))
        ...
        try:
            ...
            self.sign.setText('xStep 1: goto Capture Window')
            self.sign.repaint()

            print('xStep 1: goto Capture Window')

            time.sleep(2)
            m.click(mouse_left)
            self.sign.setText('xStep 2: Mouse')
            self.sign.repaint()
            print('xStep 2: Mouse')

            time.sleep(2)
            m.position = (pos_x, pos_y)
            self.sign.setText('xStep 3: Position')
            self.sign.repaint()
            print('xStep 3: Position')

            print('xStep 4: Start Capture... Cap:0')
            time.sleep(2)
